...
contract WalletFactoryTest is Test {
    ...
    function testDeployWallet() public {
        TrueWallet wallet = factory.deployWallet(address(entryPoint), walletOwner, salt);

        address computedWalletAddress = factory.computeAddress(address(entryPoint), walletOwner, salt);
    ...
    function testDeployWalletInCaseAlreadyDeployed() public {
        address walletAddress = factory.computeAddress(address(entryPoint), walletOwner, salt);
        ...
        walletAddress = factory.computeAddress(address(entryPoint), walletOwner, salt);
        // Determine if a wallet is already deployed at this address
        codeSize = walletAddress.code.length;
        assertTrue(codeSize > 0);
        // Return the address even if the account is already deployed
        TrueWallet wallet2 = factory.deployWallet(address(entryPoint), walletOwner, salt);
